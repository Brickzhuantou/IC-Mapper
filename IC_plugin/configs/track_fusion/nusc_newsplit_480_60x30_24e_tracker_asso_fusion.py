...
model = dict(
    ...
    head_cfg=dict(
        ...
        # 冻结检测跟踪相关
        freeze_MapTransformer=False,  #TODO:
        freeze_track_association=False,
        freeze_bn=False,
        vis_save_dir='./work_dirs/vis_global_train_pred_v3-1/test.png',
        streaming_cfg=dict(
            streaming=True,
        ...
        pointtransformer=dict(
            ...
            decoder=dict(
                type='MapTransformerPointDecoder',
                num_layers=3,
                # prop_add_stage=1,
                return_intermediate=True,
                transformerlayers=dict(
...
test_pipeline = [
    ...
    dict(type='Collect3D', keys=['img', 'vectors'], meta_keys=(
        'token', 'ego2img', 'sample_idx', 'ego2global_translation',
        'ego2global_rotation', 'img_shape', 'scene_name', 'img_filenames'))
]
...
optimizer = dict(
    type='AdamW',
    lr=5e-4 * (batch_size / (4)) *num_gpus/8 * 0.6,   # DONE: 微调阶段学习率；
    paramwise_cfg=dict(
        custom_keys={
...
# evaluation = dict(interval=num_epochs_single_frame*num_iters_per_epoch*num_epochs)
